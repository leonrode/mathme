...
export default [
...

  {
    ...
    generate: () => {
      ...
      // square = 9
      // solution = square +- random int
      // lhs = x -+ same random int
      //sqrt both sides
      const v = randomIntInRange(11, 25);
      const solution = square + v;
      const symbol = "x";

      let lhs = nerdamer(symbol);
      let rhs = nerdamer(solution);

      const op = randomAddOrSub();

      const v1 = randomIntInRange(-10, 10, [0]);
